...
for a in articles:
    cursor.execute('''
        ...
        WHERE id = ?
    ''', (
        a['method'], a['title'], a['content'], a['difficulty'],
        a.get('star_s', ''), a.get('star_t', ''), a.get('star_a', ''), a.get('star_r', ''),
        a.get('prep_p', ''), a.get('prep_r', ''), a.get('prep_e', ''), a.get('prep_p2', ''),
        a.get('keywords', ''),
        a['id']
    ))
    print(f"Updated article {a['id']}: {a['title']} ({len(a['content'])} chars)")
